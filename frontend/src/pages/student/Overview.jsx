...
export default function Overview() {
  ...
  return (
    <div className="stack">
      ...
      {focus.length > 0 && (
        <Card title="AI recommended for you"
          subtitle="Study the material, then practise the concept. Both are selected from your current learning state."
          right={<Link className="btn btn-sm" to="/app/practice">All recommendations</Link>}>
          <div className="grid grid-2">
            {focus.map((item) => (
              <div key={item.concept} className="card"
                ...
                <div className="tiny muted" style={{ margin: '.2rem 0 .4rem' }}>
                  {Math.round(item.mastery)}% mastery
                </div>
                <Bar value={item.mastery} />
                <p className="small" style={{ margin: '.6rem 0' }}>{item.reason}</p>
                <div className="row wrap">
                  <Link className="btn btn-sm" to="/app/resources">
                    Study material
                    {item.resources?.length ? ` (${minutes(item.resources.reduce((a, r) => a + r.estimated_minutes, 0))})` : ''}
                  </Link>
                  <Link className="btn btn-primary btn-sm" to="/app/practice">
                    Practise {item.available_questions || item.recommended_questions} {item.difficulty} questions
                  </Link>
                </div>
              </div>
            ))}
      ...
      {totals?.attempted > 0 && (
        <div className="grid grid-2">
          <Card title="Practice so far"
            right={<Link className="btn btn-sm" to="/app/practice-history">Full history</Link>}>
            <div className="grid grid-3">
              <StatCard label="Attempted" value={totals.attempted} />
              <StatCard label="Accuracy" value={totals.accuracy} suffix="%"
                tone={totals.accuracy >= 70 ? 'var(--good)' : totals.accuracy >= 50 ? 'var(--warn)' : 'var(--bad)'} />
              <StatCard label="Score" value={totals.score} suffix={`/ ${totals.max_score}`} />
            </div>
          </Card>
          ...
          <Card title="Recent sessions">
            <div className="stack" style={{ gap: '.35rem' }}>
              {recent.map((s) => (
                <div key={s.session_id} className="between small"
                  style={{ padding: '.4rem .55rem', border: '1px solid var(--border)', borderRadius: 8 }}>
                  <span>{s.label || s.concept || 'Mixed'}</span>
                  <span className="tiny muted mono">
                    {s.correct}/{s.questions} &middot; {s.accuracy}%
                  </span>
                </div>
              ))}
              {!recent.length && <span className="tiny muted">No completed sessions yet.</span>}
            </div>
          </Card>
        </div>
      )}
